      ...
      <div className="mt-6 rounded-[1.6rem] border border-black/6 bg-[rgba(248,243,236,0.95)] p-5">
        ...
        {editable ? (
          <textarea
            ...
            className="mt-4 min-h-36 w-full rounded-[1.2rem] border border-[#d8c5b4] bg-white px-4 py-3 text-base leading-7 text-[#3a281d] outline-none"
          />
        ) : (
          <p className="mt-4 text-base leading-7 text-[#3a281d]">{templateText}</p>
        )}
      </div>
      {actionArea ? <div className="mt-6">{actionArea}</div> : null}
    </section>
  );
}
